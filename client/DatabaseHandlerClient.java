    ...
    */
    public DatabaseHandlerClient(String path, String user, String password) throws SQLException{
        try {
            ...
            if(path.endsWith("/")) {
                conn = DriverManager.getConnection("jdbc:h2:" + path + user + ";CIPHER=AES", user, "filepwd " + password+ ";TRACE_LEVEL_FILE=0");
            }else{
                path = path.substring(0,path.length()-6);
                conn = DriverManager.getConnection("jdbc:h2:file:" + path + ";CIPHER=AES;IFEXISTS=TRUE;", user,  "filepwd " + password+ ";TRACE_LEVEL_FILE=0");
                /*
